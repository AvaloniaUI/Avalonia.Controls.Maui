...
public partial class MainPage : ContentPage
{
    ...
    {
        if (animationCancellationTokenSource?.IsCancellationRequested == true)
    ...
    async void OnRandomizeButtonClicked(object sender, EventArgs args)
    {
        ...
        {
            // Animation was cancelled, just return
            return;
        }
        finally
        {
        ...
        // Only start playing if we weren't cancelled
        if (!cancellationToken.IsCancellationRequested)
        {
            // Prepare for playing.
    ...
    protected override void OnDisappearing()
    {
        ...
        // Cancel any ongoing randomization animation
        animationCancellationTokenSource?.Cancel();

        // Stop the playing state and timer
        isPlaying = false;
    }
}
